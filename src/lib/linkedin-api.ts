...
export class LinkedInApiClient {
  ...
  /**
   * Searches the LinkedIn Ad Library for public ads.
   * Requires the LinkedIn Ad Library product to be added to the app.
   */
  async searchAdLibrary(options: {
    ...
    payerName?: string;
    countries?: string[];
    dateRange?: { start: { day: number; month: number; year: number }; end: { day: number; month: number; year: number } };
    includedTargetingFacetCategories?: string[];
    excludedTargetingFacetCategories?: string[];
    totalImpressionsRange?: { from?: number; to?: number };
    sortByField?: string;
    ...
    if (options.advertiser) params['advertiser'] = options.advertiser;
    if (options.payerName) params['payerName'] = options.payerName;
    if (options.start !== undefined) params['start'] = options.start;
    if (options.count !== undefined) params['count'] = Math.min(options.count, 25);

    if (options.countries?.length) {
      params['countries'] = options.countries.map(c => `urn:li:country:${c.toLowerCase()}`);
    }

    if (options.dateRange) {
      const { start, end } = options.dateRange;
      params['dateRange'] = `(start:(day:${start.day},month:${start.month},year:${start.year}),end:(day:${end.day},month:${end.month},year:${end.year}))`;
    }

    if (options.includedTargetingFacetCategories?.length) {
      params['includedTargetingFacetCategories'] = options.includedTargetingFacetCategories;
    }

    if (options.excludedTargetingFacetCategories?.length) {
      params['excludedTargetingFacetCategories'] = options.excludedTargetingFacetCategories;
    }

    if (options.totalImpressionsRange) {
      const { from, to } = options.totalImpressionsRange;
      const parts: string[] = [];
      if (from !== undefined) parts.push(`from:${from}`);
      if (to !== undefined) parts.push(`to:${to}`);
      params['totalImpressionsRange'] = `(${parts.join(',')})`;
    }

    if (options.sortByField) {
      params['sortBy.field'] = options.sortByField;
      params['sortBy.order'] = options.sortByOrder || 'DESCENDING';
    }

    return await this.request<{ elements: unknown[]; paging?: unknown }>('/rest/adLibrary', {
      restliMethod: 'FINDER',
      params,
    });
  }
